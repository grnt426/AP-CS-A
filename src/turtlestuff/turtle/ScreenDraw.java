 ...
 */
public final class ScreenDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
	...
	private static ScreenDraw std = new ScreenDraw();
	...
	private static Image getImage(String filename) {
		...
		if ((icon == null) || (icon.getImageLoadStatus() != MediaTracker.COMPLETE)) {
			URL url = ScreenDraw.class.getResource(filename);
	...
	public void actionPerformed(ActionEvent e) {
		FileDialog chooser = new FileDialog(ScreenDraw.frame, "Use a .png or .jpg extension", FileDialog.SAVE);
		chooser.setVisible(true);
		String filename = chooser.getFile();
		if (filename != null) {
			ScreenDraw.save(chooser.getDirectory() + File.separator + chooser.getFile());
		}
	}
	...
	public void mousePressed(MouseEvent e) {
		synchronized (mouseLock) {
			mouseX = ScreenDraw.userX(e.getX());
			mouseY = ScreenDraw.userY(e.getY());
			mousePressed = true;
		}
	}
	...
	public static void main(String[] args) {
		ScreenDraw.square(.2, .8, .1);
		ScreenDraw.filledSquare(.8, .8, .2);
		ScreenDraw.circle(.8, .2, .2);

		ScreenDraw.setPenColor(ScreenDraw.BOOK_RED);
		ScreenDraw.setPenRadius(.02);
		ScreenDraw.arc(.8, .2, .1, 200, 45);

		// draw a blue diamond
		ScreenDraw.setPenRadius();
		ScreenDraw.setPenColor(ScreenDraw.BOOK_BLUE);
		double[] x = { .1, .2, .3, .2 };
		double[] y = { .2, .3, .2, .1 };
		ScreenDraw.filledPolygon(x, y);

		// text
		ScreenDraw.setPenColor(ScreenDraw.BLACK);
		ScreenDraw.text(0.2, 0.5, "black text");
		ScreenDraw.setPenColor(ScreenDraw.WHITE);
		ScreenDraw.text(0.8, 0.8, "white text");
	}
